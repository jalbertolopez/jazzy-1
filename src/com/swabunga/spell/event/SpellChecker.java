...
public class SpellChecker {
	...
	/**
	 * This method clears the words that are currently being remembered as Ignore
	 * All words and Replace All words.
	 */
	public void reset() {
		ignoredWords.clear();
	...
	/**
	 * This method will fire the spell check event and then handle the event action
	...
	/**
	 * This method is called to check the spelling of the words that are returned by
	 * the WordTokenizer.
	 * <p>
		...
		while (tokenizer.hasMoreWords() && !terminated) {
			...
			if (!dictionary.isCorrect(word)) {
				if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE)
						&& isMixedCaseWord(word, tokenizer.isNewSentance()))
					...
					if (!ignoredWords.contains(word)) {
						...
						if (autoReplaceWords.containsKey(word)) {
							...
							SpellCheckEvent event = new BasicSpellCheckEvent(word,
									dictionary.getSuggestions(word, config.getInteger(Configuration.SPELL_THRESHOLD)),
									tokenizer);
				...
				if ((!config.getBoolean(Configuration.SPELL_IGNORESENTANCECAPITALIZATION))
						&& (tokenizer.isNewSentance()) && (Character.isLowerCase(word.charAt(0)))) {
					errors++;
